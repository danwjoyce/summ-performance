...
Given the model, we can estimate uncertainty on this prediction by computing a **credible interval** -- for example, the 80 percent (or 0.8) credible interval for the above patient's prediction is the number of samples $y^{(m)}_{new}$ in the quantile [0.1, 0.9]. The credible interval can be interpreted as:
  ...
  * there is an 80% probability that the value of $y_{new}$ is in the interval [`r round( quantile( dist.pt.A$samples, prob = c(0.1) ), 2 )`, `r round( quantile( dist.pt.A$samples, prob = c(0.9) ), 2 )`]
...
ggplot( ) +
  ...
  geom_vline( xintercept = tau, colour = "black", linetype = 2 ) +
...
Each patient is assigned a predicted value $y$ as shown in the plot above (note, the point-predictions shown are in fact the means of the full posterior distributions, discussed in Section 6.2 below).  The black dotted line represents the optimal operating threshold (cutoff) that yielded the SMP reported for the trained model.  Using this scheme, we can state:
...
theme_inset <- function (base_size = 28, base_family = "Helvetica", 
...
hist.pt.A <- ggplot(df.3) + geom_histogram( aes(A), binwidth = 0.01, fill = pt.A.col ) +
               ...
               geom_vline(aes(xintercept=tau), colour="black", linetype=2, size = 4.0) + 
               ggtitle(label = "A") +
               xlab("") + ylab("") +
               theme_inset()
...
# # EPS version for journal
# ggsave( hist.pt.A, file="../Figures/pt.A.hist.eps", device="eps", dpi = "print")
# ggsave( hist.pt.B, file="../Figures/pt.B.hist.eps", device="eps", dpi = "print")
# ggsave( hist.pt.C, file="../Figures/pt.C.hist.eps", device="eps", dpi = "print")
...
The black dotted line again shows the cutoff used to derive the SMP quoted above.

Now, we can answer the following questions relevant to the predictions for each patient using the samples $y^{(m)}$ for A, B, and C.

Beginning with the traditional cut-off threshold:

  * The **probability** that Patient A is **positive** is the number of samples $y^{(m)}_{A}$ greater than the cutoff -- it can clearly be seen that the mass of the posterior distribution for A is concentrated far below the cutoff, so the probability of being positive is 0 and Patient A is negative.
  * The **probability** that Patient B is **positive** is the number of samples $y^{(m)}_{B}$ greater than the cutoff = `r round( length( which( df.3$B > tau ) ) / length( df.3$B ), 3)` -- the probability that Patient B is positive is approximately 0.5, or chance-level
  * The **probability** that Patient C is **positive** is the number of samples $y^{(m)}_{C}$ greater than the cutoff -- clearly, the mass of the posterior distribution for Patient C is concentrated far above the cutoff (with no samples less than the cutoff), so Patient C is positive with probability 1.
...
ggplot( ) +
  ...
  annotate("segment", x = point.pred$P.Y[3], xend = point.pred$P.Y[3], y = 0, yend = 1.0, colour = pt.C.col, size = 1.5, lineend = "round") +
    ...
    # add in cutoff 
  annotate(x = tau, xend=tau, y=0, yend=tau, colour="black", lwd=0.75, geom="segment", linetype = 2 ) +
  annotate(x = 0, xend=tau, y=tau, yend=tau, colour="black", lwd=0.75, geom="segment", linetype = 2 ) +
    
  # the calibration curve
  geom_line( data = point.calib, 
             mapping = aes( x = Pred.Y, y = P.Y ), alpha = 1.0, colour = "#fed98e", size = 1.0 ) +
...
Inspecting the calibration plot, we can see that:
  ...
  * Near the decision-rule cutoff of `r tau` (the black dotted lines) the model appears well-calibrated
...
for( j in 1:nrow(calib.theta.m) ){
  ...
  calib.theta.m$pred.Y[j] <- PrY1( this.alpha, this.beta_B, this.beta_Age, this.beta_DAP, 
                                   test.df$B[j], test.df$Age[j], test.df$DAP[j] )
}
...
ComputeMedian <- function( X ) {
 ...
 # -- now, we can use the upper / lower quantiles to compute the range of calibration
 this.calib.upper <- round( Calibrate.Lowess( actual.Y, m.h$upper, 1, smooth.value ), 2 )
 this.calib.lower <- round( Calibrate.Lowess( actual.Y, m.h$lower, 1, smooth.value ), 2 )
    
 calib.quant <- left_join( this.calib.lower, this.calib.upper, by = "Pred.Y" )
 # filter obvious duplicated rows from the join
 calib.quant <- calib.quant[ duplicated( calib.quant ), ]
 # remove rows with NAs
 calib.quant <- calib.quant[ complete.cases( calib.quant ), ]
 # rename for clarity
 names( calib.quant ) <- c("P.Y.upper", "Pred.Y", "P.Y.lower")
...
calib.spag.p <- ggplot( ) +
  ...
  geom_ribbon( data = calib.quant, aes( x = Pred.Y, ymin = P.Y.lower, ymax = P.Y.upper ), fill = "#fed98e") +
...
# For journal version  
# tiff( '../Figures/composite_diag1.tif', width = 800, height = 800, units = "px" )
#   print( calib.spag.p )
#   invisible( dev.off() )
...
# need to save an EPS version for print journal 
# ggsave(file="../Figures/calib_diag.eps", device="eps", dpi = "print")

```

In the above plot, the orange ribbon represents the 80% credible interval of the collection of $M$ calibration curves (rather than over-plot every calibration curve for a spaghetti plot).  The black dotted line shows the cutoff (0.4) and the three coloured lines represent the median prediction (derived from the posterior distributions of Section 6.2).  
